...
int main(int argc, char* argv[]) {
	...
	//--Obtenemos el string del nombre y dirección del orquestador
	...
	if ((sockOrq = socket(AF_INET, SOCK_STREAM, 0)) != 0) {
		...
		enviaMensaje(sockOrq, &orqMsj, sizeof(orq_t), logger, "Saludo Orq");
	}
	...
	while (1) {
		...
		if (i != -1) {
			...
			case SALUDO:
				...
				enviaMensaje(i, &message, sizeof(message), logger, "Pos inicial");
				break;
			case POSICION:
				...
				enviaMensaje(i, &message, sizeof(message), logger, "Pos recurso");
				break;
			case MOVIMIENTO:
				//--Obtiene posición del item pedido y del personaje
				getPosRec(ListaItems, message.detail2, &posItemX, &posItemY);
				...
				if ((posItemX == posX) && (posItemY == posY)) {
					...
					for (contPj = 0; contPj < list_size(personajes); contPj++) {
						...
						if (tempAux->numSock == i) {
							list_add_new(tempAux->recursos, &(message.detail2), sizeof(message.detail2));
							break;
						}
					}

					//--Resta uno a la cantidad del item
					int result = restarQuantityRec(ListaItems, message.detail2);

					if (result >= 0) {

						log_info(logger, "A %d se le da el recurso %c", i, message.detail2);
						...
						enviaMensaje(i, &message, sizeof(message), logger, "Confirma pedido");
					} else {
						//--Si el recurso ya no tiene instacias
						tempAux->blocked = true;

						log_info(logger, "A %d se le niega el recurso %c", i, message.detail2);
						//--Define rechazo y lo devuelve
						message.detail2 = message.detail;
						message.detail = BLOCK;
						enviaMensaje(i, &message, sizeof(message), logger, "Confirma bloqueo");
					}
				} else {
					//--Define confirmación y la devuelve
					...
					message.detail2 = message.detail;
					message.detail = NADA;
					enviaMensaje(i, &message, sizeof(message), logger, "Confirma movimiento");
				}
...
// Iteramos infinitamente
	while (1) {
		...
		if ((list_size(bloqueados) > 1) && (parametros.recovery)) {
			...
			enviaMensaje(parametros.numSock, &header, sizeof(header), logger, "Envia header DDL");
			...
			while (list_size(bloqueados) != 0) {
				...
				enviaMensaje(parametros.numSock, &message, sizeof(message), logger, "Envia personajes DDL");
...
void notificarPersonajes() {
	...
	while (i < list_size(personajes)) {	//Cicla los personajes
		...
		enviaMensaje(personaje->numSock, &message, sizeof(message), logger, "Muerte del personaje");
		i++;
	}
}
